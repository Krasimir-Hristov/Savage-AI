...
- [ ] **Step 0.1: Create `.github/copilot-instructions.md`**
  Rules covering:
...
- [ ] **Step 1.2: Install core dependencies**
  ...
  - `shadcn` (init + core components: button, input, card, dialog, avatar, scroll-area, dropdown-menu, sheet, skeleton, toast, badge)
  - `zod` (validation)
  - `lucide-react` (icons)
...
- [ ] **Step 1.4: Set up project folder structure**
  ...
  │   │   │   └── page.tsx          # New chat / chat list
  │   │   ├── characters/page.tsx
  │   │   └── layout.tsx
  │   ├── api/
  │   │   └── chat/route.ts         # OpenRouter streaming endpoint
  │   ├── layout.tsx               # Root layout
  │   ├── page.tsx                 # Landing page
  │   ├── proxy.ts                 # Auth proxy (Next.js 16 replaces middleware)
  │   ├── error.tsx
  │   ├── not-found.tsx
  │   └── loading.tsx
  ├── components/
  ...
  │   ├── openrouter/
  │   │   ├── client.ts      # OpenRouter API client (server-only)
  ...
  └── hooks/
      ├── use-chat.ts        # Chat hook (streaming, messages state)
...
1. **Phase 0**: Copilot rules load correctly, AI agents follow conventions
2. **Phase 1**: `npm run dev` starts without errors, shadcn components render
3. **Phase 2**: Full auth flow works (signup → login → logout → proxy redirect)
4. **Phase 3**: Send message → receive streaming response from OpenRouter → message displays with markdown → conversation saves to DB
5. **Phase 4**: Can select character, different personalities respond differently
6. **Phase 5**: Refresh page → conversation loads → can continue chatting
7. **Manual**: Test on mobile viewport, test with slow connection, test error states
